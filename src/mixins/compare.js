export default {
  ...
  methods: {
    compareObj() {
      let _keysbefore = Object.keys(this.before);
      let _keysafter = Object.keys(this.after);
      let _temp = _keysbefore.concat(_keysafter);
      let _keys = _temp.filter((item, i, arr) => arr.indexOf(item) === i);
      console.log(_keys);
      _keys.forEach(key => {
        ...
        if (this.after[key] !== this.before[key]) {
          ...
          switch (key) {
            case 'accountpercent':
              ...
              break;
            default:
              this.after[key] = `${this.after[key] || '-'}(修改前:${this.before[key] || '-'})`;
          }
        }
      });
    },
